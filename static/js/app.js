...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function _maybeShowMaintNote() {
        ...
        document.body.appendChild(wrap);
        try { if ('Notification' in window && Notification.permission === 'granted') new Notification('Quolvex — Back Online', { body: note.slice(0, 120), icon: '/static/icons/icon-192.png', tag: 'maint-note', requireInteraction: true }); } catch (e) {}
        return true;
    }
    ...
    async function devicePush(title, body, tag) {
        try { showDiscordToast('Quolvex', title.replace('Quolvex','').replace('—','').trim() || title, body, title.charAt(0)); } catch {}
        // Real OS notification: works whenever the Quolvex tab is open even if the user is on
        // another website/window — shows in the Windows action center / mobile notification panel.
        try {
            if ('Notification' in window && Notification.permission === 'granted') {
                const n = new Notification('Quolvex — ' + (title || 'Quolvex'), { body: String(body || '').slice(0, 200), icon: '/static/icons/icon-192.png', tag: tag || 'zenith', requireInteraction: true });
    ...
    // Hide Invite button if user has earned Pro from referrals
    try {
        const refStats = await api('/api/referral/stats');
        if (refStats.has_pro || refStats.earned_from_referrals) {
    ...
    $('invite-btn').addEventListener('click', async () => {
        ...
        wrap.innerHTML = `
            <div style="background:linear-gradient(160deg,#1c0d40,#170a33 60%,#12082b);border:1px solid rgba(139,92,246,.55);border-radius:20px;padding:32px;width:100%;max-width:440px;box-shadow:0 10px 40px rgba(0,0,0,0.6),0 0 50px rgba(139,92,246,.25);position:relative;overflow:hidden;">
                <div style="position:absolute;top:0;left:8%;right:8%;height:2px;background:linear-gradient(90deg,transparent,#22d3ee,#ff3db4,#ffd23f,transparent);"></div>
                <button id="invite-close" style="position:absolute;top:12px;right:12px;background:none;border:none;color:#8b7fb8;font-size:20px;cursor:pointer;">&times;</button>
                <div style="text-align:center;margin-bottom:20px;">
                    <div style="width:58px;height:58px;margin:0 auto 12px;border-radius:18px;background:linear-gradient(135deg,#ff3db4,#8b5cf6,#22d3ee);background-size:200% 200%;animation:invOrb 6s ease infinite;display:flex;align-items:center;justify-content:center;font-size:26px;box-shadow:0 8px 26px rgba(255,61,180,.5);">&#127968;</div>
                    <h2 style="background:linear-gradient(90deg,#22d3ee,#ff3db4 55%,#ffd23f);-webkit-background-clip:text;background-clip:text;color:transparent;font-size:21px;font-weight:800;margin:0 0 6px;">Invite Friends</h2>
                    <p style="color:#b6a8e0;font-size:13px;margin:0;">Invite 5 friends and get <strong style="color:#ffd23f;">Pro for 3 days</strong> free!</p>
                </div>
                <div id="invite-stats" style="display:flex;gap:12px;margin-bottom:20px;">
                    <div style="flex:1;text-align:center;padding:12px;background:rgba(34,211,238,.08);border:1px solid rgba(34,211,238,.3);border-radius:12px;box-shadow:0 0 14px rgba(34,211,238,.08);">
                        <div id="invite-count" style="font-size:24px;font-weight:800;color:#7df0ff;text-shadow:0 0 14px rgba(34,211,238,.5);">0</div>
                        <div style="font-size:11px;color:#8b7fb8;">Joined</div>
                    </div>
                    <div style="flex:1;text-align:center;padding:12px;background:rgba(255,210,63,.08);border:1px solid rgba(255,210,63,.3);border-radius:12px;box-shadow:0 0 14px rgba(255,210,63,.08);">
                        <div id="invite-pending" style="font-size:24px;font-weight:800;color:#ffd23f;text-shadow:0 0 14px rgba(255,210,63,.5);">0</div>
                        <div style="font-size:11px;color:#8b7fb8;">Remaining</div>
                    </div>
                    <div style="flex:1;text-align:center;padding:12px;background:rgba(139,92,246,.1);border:1px solid rgba(139,92,246,.35);border-radius:12px;box-shadow:0 0 14px rgba(139,92,246,.1);">
                        <div id="invite-rewarded" style="font-size:24px;font-weight:800;color:#c4b5fd;text-shadow:0 0 14px rgba(139,92,246,.5);">0</div>
                        <div style="font-size:11px;color:#8b7fb8;">Rewarded</div>
                    </div>
                </div>
                <div style="margin-bottom:20px;">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                        <span style="font-size:12px;color:#b6a8e0;">Progress to Pro</span>
                        <span id="invite-progress-text" style="font-size:12px;color:#ffd23f;font-weight:700;">0/5</span>
                    </div>
                    <div style="width:100%;height:8px;background:rgba(139,92,246,.15);border-radius:4px;overflow:hidden;">
                        <div id="invite-progress-bar" style="height:100%;width:0%;background:linear-gradient(90deg,#22d3ee,#ff3db4,#ffd23f);background-size:200% 100%;animation:vsxg 4s ease infinite;border-radius:4px;transition:width 0.3s;box-shadow:0 0 12px rgba(255,61,180,.5);"></div>
                    </div>
                </div>
                ...
                <div style="display:flex;gap:8px;margin-bottom:16px;">
                    <button id="invite-twitter" style="flex:1;padding:10px;background:#1DA1F2;color:white;border:none;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 4px 14px rgba(29,161,242,.4);">Share on Twitter</button>
                    <button id="invite-whatsapp" style="flex:1;padding:10px;background:#25D366;color:white;border:none;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 4px 14px rgba(37,211,102,.4);">Share on WhatsApp</button>
                </div>
                <button id="invite-claim" style="display:none;position:relative;overflow:hidden;width:100%;padding:12px;background:linear-gradient(135deg,#ff3db4,#8b5cf6 55%,#22d3ee);background-size:180% 180%;color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 4px 20px rgba(255,61,180,.45);animation:vsxg 5s ease infinite;">✨ Claim Pro Reward</button>
            </div>`;
        document.body.appendChild(wrap);
        ...
        // Load stats
        try {
            const [stats, linkData] = await Promise.all([
                api('/api/referral/stats'),
                api('/api/referral/link')
            ]);
            wrap.querySelector('#invite-count').textContent = stats.total;
            wrap.querySelector('#invite-pending').textContent = Math.max(0, stats.required - stats.total);
            wrap.querySelector('#invite-rewarded').textContent = stats.rewarded;
            wrap.querySelector('#invite-link').value = linkData.link;
            const pct = Math.min(100, (stats.total / stats.required) * 100);
            wrap.querySelector('#invite-progress-bar').style.width = pct + '%';
            wrap.querySelector('#invite-progress-text').textContent = `${stats.total}/${stats.required}`;
            if (stats.has_pro) {
                // Already earned Pro — hide claim, show message
                wrap.querySelector('#invite-claim').style.display = 'none';
            } else if (stats.total >= stats.required) {
                ...
                claimBtn.addEventListener('click', async () => {
                    try {
                        ...
                        // Hide invite button entirely after claiming
                        const inviteBtn = $('invite-btn');
                        if (inviteBtn) inviteBtn.style.display = 'none';
    ...
    (function reminders() {
        ...
        async function renderReminders() {
            try {
                ...
                list.innerHTML = '';
                if (!reminders.length) {
                    list.innerHTML = `<div style="color:var(--muted); font-size:13px; text-align:center; padding:18px;">No reminders yet. Quolvex will pop them here for you.</div>`;
                    return;
                }
        ...
        window.__handlePendingReminder = (msg) => {
            ...
            if (typeof Notification === 'function' && Notification.permission === 'granted') {
                try { new Notification('Quolvex reminder', { body: msg, icon: '/favicon.ico' }); } catch (e) {}
            }
        };
        void origNotif;
    })();
    ...
    async function refreshFeedbackBadge() {
        ...
        try {
            ...
            if (unanswered > 0) {
                ...
                if (unanswered > lastFeedbackCount) { showToast(`New feedback: ${unanswered} unanswered`, ''); devicePush('Quolvex Feedback', `${unanswered} new feedback awaiting reply`, 'feedback-admin'); showDiscordToast('Quolvex', 'New Feedback', `${unanswered} awaiting reply`, 'F'); }
            ...
            if (unseen > 0) {
                ...
                if (unseen > 0 && replied > seen) {
                    ...
                    if (latest && !document.getElementById('feedback-reply-popup')) {
                        ...
                        devicePush('Quolvex Feedback Reply — ' + who, latest.response, 'feedback-reply');
                        showDiscordToast('Quolvex', who, latest.response.slice(0,80), who.charAt(0));
                    }
                }
            }
            else feedbackBadge.style.display = 'none';
    ...
    async function refreshRequestBadge() {
        ...
        try {
            ...
            if (unanswered > 0) {
                ...
                if (unanswered > lastRequestCount) { showToast(`New personal request: ${unanswered} awaiting reply`, ''); devicePush('Quolvex Personal Request', `${unanswered} new request awaiting reply`, 'request-admin'); showDiscordToast('Quolvex', 'New Personal Request', `${unanswered} awaiting reply`, 'R'); }
            ...
            if (unseen > 0) {
                ...
                if (repliedCount > seen) {
                    ...
                    if (latest && !document.getElementById('request-reply-popup')) {
                        ...
                        devicePush('Quolvex Request Reply — ' + who, latest.reply, 'request-reply');
                        showDiscordToast('Quolvex', who, latest.reply.slice(0,80), who.charAt(0));
                    }
                }
            } else requestBadge.style.display = 'none';
        } catch (e) {}
            ...
            if (anns.length) {
                ...
                if (pending.length) {
                    for (const a of pending) {
                        ...
                        try { devicePush('Quolvex Broadcast — ' + a.username, a.content, 'broadcast-' + a.id); } catch (e) {}
                        try { showDiscordToast('Quolvex', a.username + ' — Broadcast', a.content.slice(0, 80), a.username); } catch (e) {}
                    }
    ...
    function showBroadcastPopup(a) {
        ...
        if (emMatch) {
            ...
            if (t === 'maintenance' || t === 'lock-all') {
                ...
                try { devicePush('Quolvex — Unlocked', 'All accounts have been unlocked by the Owner.', 'unlock-all'); } catch (e) {}
                _persistentScreen = '';
                const sc = document.getElementById('maintenance-screen');
                if (sc) sc.remove();
            } else {
                window.showEmergencyPopup(t);
                try { devicePush('Quolvex — ' + (t === 'registrations' ? 'Registrations Closed' : t === 'messaging' ? 'Messaging Disabled' : t === 'ai' ? 'AI Disabled' : 'System Update'), a.content.replace(/^\[EMERGENCY:\w+\]\s*/, ''), 'emergency-' + t); } catch (e) {}
            }
            return;
        }
        ...
        try {
            ...
            if (newMsgs.length && _lastStaffId !== 0) {
                newMsgs.forEach(m=>{
                    ...
                    showDiscordToast('Quolvex', m.username, m.content.slice(0,80), m.username);
                ...
                if (latest.username !== user.username) { showDiscordToast('Quolvex', latest.username, latest.content.slice(0,80), latest.username); devicePush('Staff Live Chat — ' + latest.username, latest.content.slice(0,120), 'livechat-'+latest.id); try { const a=new Audio('/static/sounds/notify.mp3'); a.volume=0.6; a.play().catch(()=>{}); } catch {} }
            }
...
function showBannedScreen(reason, bannedBy) {
    ...
    const banLine = isOwnerBan
        ? 'By the decree of <strong style="color:#C0C7D1;">The Owner</strong> — WANZU-IBRAHIM. Your access to Quolvex has been revoked, effective immediately.'
...
function showDeletedScreen(deletedBy) {
    ...
    const line = isOwner
        ...
        : 'Your account has been deleted by an administrator. You can no longer access Quolvex.';
    div.innerHTML = `
