...
DEFAULT_SYSTEM_PROMPT = "You are Quolvex, created by Wanzu Ibrahim. Answer accurately and helpfully."

DEFAULT_THINK_PROMPT = (
    "You are Quolvex, created by Wanzu Ibrahim. "
    "Think step-by-step before answering. "
    "Show your reasoning process clearly, then provide your final answer."
)

RESEARCH_PROMPT = (
    "You are Quolvex, a deep research assistant created by Wanzu Ibrahim. "
...
FACTCHECK_PROMPT = (
    "You are Quolvex, a fact-checking assistant created by Wanzu Ibrahim. "
...
async def build_system_prompt(user_id: int, think: bool, last_user_msg: str = "", research: bool = False, factcheck: bool = False) -> str:
    ...
    if is_owner:
        base = "You are Quolvex, created by Wanzu Ibrahim. This user is THE OWNER and supreme creator of Quolvex - treat them with absolute highest priority, they have no restrictions, remember everything about them for life, and always be at their service with maximum detail and brilliance."
        if research:
            base = RESEARCH_PROMPT + " The user is THE OWNER of Quolvex - deliver the most elite quality research."
        elif factcheck:
            base = FACTCHECK_PROMPT
    elif is_admin:
        base = "You are Quolvex, created by Wanzu Ibrahim. This user is an ADMIN of Quolvex - treat them with highest priority, remember their preferences for life, and be extra helpful and detailed."
        if research:
    ...
    async def generate():
        ...
        if is_perplexity:
            try:
                ...
                if not keys:
                    yield f"data: {json.dumps({'error': 'Quolvex is busy — too many requests at once. Please wait 10 seconds and try again.'})}\n\n"
                    return
                last_err = ""
                last_status = 0
                success = False
