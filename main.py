...
USER_CHANGELOG = [
    "20.0 Quolvex AI: everything you loved is still here — same features, same chats — under a brand-new name. Say hello to Quolvex AI!",
    "Files finally open up to the AI: upload a PDF, Word, Excel, PowerPoint, audio or video and Quolvex AI actually reads it — it can summarize, answer questions about, edit, replicate and rewrite whatever you send",
    ...
    "Videos get processed too — Quolvex AI grabs key frames from the video so it can describe what's on screen, plus audio transcription when a transcription key is configured",
    ...
    "Voice speed control: tune how fast Quolvex AI speaks the replies out loud",
    ...
    "Just ask for an image: type something like 'generate a pic of...' and Quolvex AI creates it automatically",
    ...
    "Personality Mirror: Quolvex AI studies how you write and matches your tone, energy and vibe in its replies (tunable in Settings → System Prompt)",
    "19.6 busy-proof downloads: if Quolvex AI is busy when you ask for a file/doc generation, you get a test file you can still download (choose md/docx/pdf/pptx/xlsx/csv) to verify downloads work",
...
STAFF_CHANGELOG = [
    *USER_CHANGELOG,
    "20.0 Quolvex AI rebrand: user-visible 'Zenith' branding replaced with Quolvex (titles, headers, meta/og tags, About text, share/invite/status pages); functional identifiers unchanged (zenith_token cookie, zenith_device_id localStorage, DB filename, Railway domain) so sessions/logins keep working; VERSION -> 20.0",
...
app = FastAPI(title="Quolvex AI", version=VERSION, lifespan=lifespan)
...
async def validate_invite_token(request: Request, db=Depends(get_db)):
    ...
    # Device already joined Quolvex AI → it can never use another invite link.
        ...
        if used:
            ...
            if owner and not owner.is_deleted:
                return {
                    ...
                    "detail": "This device already joined Quolvex AI",
