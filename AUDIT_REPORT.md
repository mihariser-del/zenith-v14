...
- **Image follow-up edits** — `image.py` `/api/image/edit`: takes `{chat_id, change, width, height}`, finds the most recent generated image in the chat (prompt + `seed=` extracted from the persisted assistant message), builds `merged = base + ", " + change` (≤500 chars; change ≤200), reuses the same seed for a similar composition, and persists `**Prompt:** … **Changed:** … ![Generated image](url)`. Each edit keeps its `**Prompt:**` line so the next edit chains onto the latest result. Guards: 400 if the chat has no generated image; counts toward the `image_gen` limit and chat media window. Client: `Chat.editGeneratedImage` + `Chat._hasGeneratedImage` (scans `#chat-container` for `![Generated image](`); auto-triggered in `send()` by `detectImageEditRequest` (app.js) when the chat already has an image — verbs (`add/remove/change/make/draw/it…`) followed by an explicit image reference ("the image", "it") or a short article-led change ("a bell", "the hat").
